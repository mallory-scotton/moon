...
/**
 * @brief Downloads the FFmpeg binary
 * @description Downloads the FFmpeg binary for the specified platform and architecture
 * @param binaryName - The name of the binary to download
 * @throws Error if the download fails
 */
async function downloadFFmpegBinary(binaryName: string) {
...
export async function getFFmpegBinaryPath(): Promise<string> {
  ...
  const binaryName = `ffmpeg-${PLATFORM}-${ARCH}${PLATFORM === 'win32' ? '.exe' : ''}`;
  const binaryPath = path.join(MOON_APPDATA, 'binaries', binaryName);
  ...
  // Download the binary
  await downloadFFmpegBinary(binaryName);
