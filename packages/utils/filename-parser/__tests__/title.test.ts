/** Dependencies */
import { parseTitleAndYear } from '../src/utils';

/**
 * @brief Test cases for the parseTitleAndYear function
 * @description This function takes a string input and returns an object containing the parsed title and year.
 */
const CASES: Array<[string, { title: string; year: number | null }]> = [
  ...
  ['Blade Runner 2049 (2017)', { title: 'Blade Runner 2049', year: 2017 }],
  ['Scarface.Anniversary.Edition.1983.INTERNAL.DVDRip.XviD-VoMiT', { title: 'Scarface', year: 1983 }],
  ['Ouija.Origin.of.Evil.2016.MULTi.TRUEFRENCH.1080p.BluRay.x264-MELBA', { title: 'Ouija Origin of Evil', year: 2016 }],
  ...
  ['Scarface.The.Uncut.Version.1983.DVDRip.Divx.AC3.iNTERNAL-FFM', { title: 'Scarface', year: 1983 }],
  ['Casino.10TH.ANNiVERSARY.1995.iNTERNAL.DVDRiP.XViD-KiSS', { title: 'Casino', year: 1995 }],
  ['Get.Him.To.The.Greek.UNRATED.FRENCH.720p.BluRay.x264-NERDHD', { title: 'Get Him To The Greek', year: null }],
  ['The.Social.Network.German.720p.BluRay.x264-DECENT', { title: 'The Social Network German', year: null }],
  ['The.Outsiders.DC.German.1983.AC3.BDRip.XviD.INTERNAL-ARC', { title: 'The Outsiders', year: 1983 }],
  ['The.Girl.in.the.Spiders.Web.2018.2160p.UHD.BluRay.x265-VALiS', { title: 'The Girl in the Spiders Web', year: 2018 }]
];
 ...
 * @description This function takes a string input and returns an object containing the parsed title and year.
 */
describe('Title and Year Parsing', () => {
  it.each(CASES)('should parse title and year from %s', (input, expected) => {
    expect(parseTitleAndYear(input)).toEqual(expected);
  });
});
