...
const CASES: [string, Partial<Edition>][] = [
  ...
  ['Warcraft.The.Beginning.3D.HOU.2016.German.DL.1080p.BluRay.x264-COiNCiDENCE', /*{ threeD: true, hou: true } NOT PROPERLY HANDLED*/ {}],
  ['Iron.Man.2008.INTERNAL.REMASTERED.2160p.UHD.BluRay.X265-IAMABLE', { uhd: true, remastered: true, internal: true }],
  ...
  ['Solo.A.Star.Wars.Story.2018.BONUS.DELETED.SCENES.1080p.BluRay.x264-PussyFoot', { deletedScenes: true }],
...
/**
 * @brief Edition test
 * @description Test the edition parsing functionality.
 */
describe('Edition extraction', () => {
  test.each(CASES)('%s', (filename, expected) => {
    const result = parseFilename(filename, false);
    expect(result.edition).toMatchObject(expected);
  });
});

/**
 * @brief Hardcoded subtitles test cases
 * @description Test cases for hardcoded subtitles detection.
 */
const HARDCODED_CASES: [string, true | undefined][] = [
  ...
  ['Movie Title! 2019 [HorribleSubs][Web][MKV][h264][848x480][AAC 2.0][Softsubs(HorribleSubs)]', undefined]
];

/**
 * @brief Hardcoded subtitles test
 * @description Test the hardcoded subtitles detection functionality.
 */
describe('Hardcoded subtitles extraction', () => {
  test.each(HARDCODED_CASES)('%s', (filename, expected) => {
    const result = parseFilename(filename, false);
    expect(result?.edition?.hardcodedSubs).toBe(expected);
  });
});
